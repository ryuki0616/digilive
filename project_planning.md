...
- **参加者管理**: 受付でNFCカードと参加者を紐づけ、ステータス（所持金含む）を管理する。
- **ステータス更新**: 各ブースでの結果（ステータス、所持金）をNFCカードにリアルタイムで書き込む。
- **ランキング**: 総合・ステータス別・**所持金**ランキングを生成し、会場モニターに表示する。
...
- **NFCカード**: ISO/IEC 14443 Type A (NTAG215相当 / NFC Forum Type 2)
...
- **ブース連携プログラム**:
  - **Python (pyscard)** にて開発。
- **連携方式**:
  - UI (Electron) とNFC制御 (Python) は `child_process` を用いて連携する。
- **総合管理・表示アプリケーション**:
  - ランキング表示、データ閲覧機能。
...
- **接続コマンド**: **[機密情報のためマスク]**
  - 内部接続: `ssh -p 22222 [ユーザー名]@[内部IPアドレス]`
  - 外部接続（VPN）: `ssh -p 22222 [ユーザー名]@[VPNIPアドレス]`
...
- **パワーチャージ・ステーション**: ボタン連打で パワー💪 / スタミナ❤️‍🔥 を上げる。（所持金も獲得可能）
- **リアクション・スプリンター**: モグラ叩き風ゲームで スピード⚡ / テクニック🎯 を上げる。（所持金も獲得可能）
- **トレジャー・ハント**: 記憶力ゲームで テクニック🧠 を上げる。（所持金も獲得可能）
- **バーチャル・ランナー**: その場足踏みで スタミナ❤️‍🔥 / スピード⚡ を上げる。（所持金も獲得可能）
- **ミラクル・タッチ**: 運試しでランダムにステータス（ラック✨含む）を上げる。（所持金も獲得可能）
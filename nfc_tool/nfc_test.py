...
# 読み取りコマンドを作成
# [0xFF, 0xB0, 0x00, 0x04, 0x04] = 「4ページ目から4バイト読み取って」
    ...
    try:
        ...
        while True:
            try:
                ...
                # 書き込みコマンドを送信
                # CMD_WRITE_PAGE: 先ほど定義した「書き込みコマンド」
                ...
                if sw1 == 0x90 and sw2 == 0x00:
                    ...
                    continue  # エラーならループの最初に戻る
                
                # 書き込み後、データが安定するまで少し待つ
                # 0.1秒（100ミリ秒）待機します
                # これにより、書き込みが完全に完了してから次の処理に進みます
                time.sleep(0.1)

                # ============================================
                # ステップ3: 書き込んだデータを読み取って確認する
                # ============================================
                ...
                # 読み取りコマンドを送信
                # CMD_READ_PAGE: 先ほど定義した「読み取りコマンド」
                ...
                if sw1 == 0x90 and sw2 == 0x00:
                    ...
                    # 書き込んだデータと読み取ったデータが一致するか確認
                    # これにより、書き込みが正しく行われたことを検証します
